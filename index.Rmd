...
In this example, we're focusing on Nepal, but you can chose any country you like. To select the country of interest, refer to the country names available [here](https://unstats.un.org/unsd/methodology/m49/).  
...
```
Because expert-based assumptions were applied to fertility in 2050, the projected trend shows a discontinuity, especially for lower education. Details about assumption is given in 
...
ggplot(surv_df %>% 
  ...
  facet_wrap(~age) +
  labs(
    title = "Survival Ratio by Education",
    x = "Year",
    y = "Survival Ratio",
    color = "Education"
  )
...
Because the COVID-19 pandemic reduced survival ratios in 2020, we observe pronounced recovery in 2025.
Education differential in mortality is only applied from the age 15 in Input data.
...
3. Since no transition data exist beyond age 10, we assume that at ages 0-4, individuals transition from no education to incomplete primary in the next 5-year interval, and at ages 5-9, they transition from incomplete primary to primary (based on India's Compulsory Education Act 2009). (everyone above age 10 will thus have at least primary education)
4. Demographic rates remain stable (time-invariant) before 2020, the earliest observed data point.

### 6.Exercise 
##### 6.1 Educatuion compositionfo of selected kin over the life course of Focal
...
E2.1 In which year can a woman born in 2020 expect to have at least half of her children attain at least a secondary level of education? 

E2.2 What does the shape of the curves tell us about the demographic processes behind  fertility timing and education progression?  
...
E3.1 Compare the educational composition of children in 2020, 2050, and 2080 for a women of age 65. What trends do you observe in the number of primary vs. secondary educated children over time? 

E3.2 At what ages do children with upper secondary & above level of education tend to appear in each year? What does this suggest about educational progression across cohorts?
...
Run the function to download data. In this case, we're downloading data for Nepal, but you can chose any country you like. To select the country of interest, refer to the country names available [here](https://unstats.un.org/unsd/methodology/m49/).  
